...
export function PlatformHero() {
  return (
    <section className="relative overflow-hidden bg-[#060816] pb-24 pt-36 text-white md:pb-32 md:pt-44">
      <div aria-hidden className="pointer-events-none absolute inset-0">
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse 80% 60% at 50% 50%, transparent 18%, rgba(6,8,22,0.72) 100%)",
          }}
        />
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          <Reveal delay={0.18}>
            <div className="mt-10 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              <Button href={platformHero.primaryCta.href} variant="glow" size="xl" trailingIcon>
                {platformHero.primaryCta.label}
              </Button>
              <Button href={platformHero.secondaryCta.href} variant="outline-white" size="xl">
                {platformHero.secondaryCta.label}
              </Button>
            </div>
          </Reveal>
          ...
          <Reveal delay={0.24}>
            <div className="relative mt-14 overflow-hidden rounded-2xl border border-white/[0.07]">
              ...
              <div className="grid grid-cols-2 divide-x divide-white/[0.06] sm:grid-cols-4">
                {platformHero.proofRow.map((item, index) => (
                  <div key={item.label} className="flex flex-col items-center gap-1 px-4 py-6">
                    ...
                    <span className="text-[0.72rem] text-white/40">{item.label}</span>
                  </div>
                ))}
              </div>
            </div>

        ...
        <Reveal delay={0.14}>
          <div className="mt-14 grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="grid gap-4 sm:grid-cols-2">
              {platformReasoningLayer.pillars.map((pillar) => (
                <div
                  key={pillar.title}
                  className="relative overflow-hidden rounded-2xl border border-white/[0.07] bg-[#040810] p-6"
                >
                  <div
                    aria-hidden
                    className="mb-4 h-px"
                    style={{
                      background: `linear-gradient(to right,${pillar.color}60,transparent)`,
                    }}
                  />
                  <h3 className="text-[0.94rem] font-semibold tracking-[-0.02em] text-frost">
                    {pillar.title}
                  </h3>
                  <p className="mt-2 text-[0.78rem] leading-relaxed text-white/45">{pillar.body}</p>
                </div>
              ))}
            </div>

            <div className="relative overflow-hidden rounded-2xl border border-white/[0.07] bg-[#040810] p-6">
              <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
                <div className="absolute -right-8 top-0 h-40 w-40 rounded-full bg-brand-500/[0.10] blur-[80px]" />
              </div>
                  ...
                  {platformReasoningLayer.comparisons.map((row) => (
                    <div
                      ...
                      className={`rounded-xl border px-4 py-4 ${
                        row.highlight
                          ? "border-brand-400/20 bg-brand-500/[0.10]"
                          : "border-white/[0.06] bg-white/[0.03]"
                      }`}
                    >
                      <div
                        className={`text-[0.8rem] font-semibold uppercase tracking-[0.12em] ${
                          row.highlight ? "text-brand-300" : "text-white/62"
                        }`}
                      >
                        {row.label}
                      </div>
                      <p
                        className={`mt-2 text-[0.78rem] leading-relaxed ${
                          row.highlight ? "text-white/60" : "text-white/42"
                        }`}
                      >
                        {row.body}
                      </p>
